        ...
        with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
            ...
            for i, f in enumerate(ghx_mandatory_fields):
                ...
                if f not in df.columns:
                    ...
                    if chart_rows == 0:
                        correct, error, empty, missing, uom_conflict = 0, 0, 1, 0, 0  # Toon minimaal 1 voor zichtbaarheid
                    else:
                        correct, error, empty, missing, uom_conflict = 0, 0, 0, chart_rows, 0
                else:
                    filled = filled_counts.get(f, 0)
                    errors = corrected_errors.get(f, 0)
                    
                    # UOM CONDITIONAL LOGIC: Voor UOM velden, check of ze conditioneel verplicht zijn
                    uom_fields = [
                        'UOM Code Verpakkingseenheid', 'Inhoud Verpakkingseenheid',
                        'UOM Code Basiseenheid', 'Inhoud Basiseenheid', 
                        'UOM Code Inhoud Basiseenheid'
                    ...
                    if f in uom_fields:
                        # Voor UOM velden: check of Omschrijving Verpakkingseenheid gevuld is
                        omschrijving_filled = filled_counts.get('Omschrijving Verpakkingseenheid', 0)
                        
                        if omschrijving_filled == 0:
                            # Geen Omschrijving Verpakkingseenheid, dus UOM velden zijn niet verplicht
                            # Toon alle gevulde UOM data als "leeg" (geel)
                            correct = 0
                            error = 0
                            empty = chart_rows  # Alle rijen als leeg
                            uom_conflict = 0
                        else:
                            # Er is wel Omschrijving Verpakkingseenheid, normale berekening
                            uom_conflict = 0
                            if not df_errors_mand.empty and 'Foutcode' in df_errors_mand.columns:
                                field_724_errors = df_errors_mand[
                                    (df_errors_mand['GHX Kolom'] == f) & 
                                    (df_errors_mand['Foutcode'] == 724)
                                ]
                                if not field_724_errors.empty and 'Rij' in field_724_errors.columns:
                                    uom_conflict = field_724_errors['Rij'].nunique()
                            
                            error = max(0, errors - uom_conflict)
                            correct = max(0, filled - errors)
                            empty = chart_rows - filled
                    else:
                        # Niet-UOM velden: normale berekening
                        uom_conflict = 0
                        if not df_errors_mand.empty and 'Foutcode' in df_errors_mand.columns:
                            field_724_errors = df_errors_mand[
                                (df_errors_mand['GHX Kolom'] == f) & 
                ...
                ws_dash.write(row_num, 5, uom_conflict)  # New UOM Conflict column
                bar_chart_data.append([f, correct, error, empty, missing, uom_conflict])
