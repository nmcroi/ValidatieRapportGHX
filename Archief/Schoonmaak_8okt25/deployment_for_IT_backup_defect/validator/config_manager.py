...
def load_institution_codes() -> Dict[str, str]:
    ...
    try:
        ...
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        # Laad server paths voor Template Generator Files directory
        try:
            from .rapport_utils import SERVER_PATHS
            tg_files_dir = SERVER_PATHS.get("template_generator_files", "Template Generator Files")
        except ImportError:
            tg_files_dir = "Template Generator Files"
        codes_path = os.path.join(base_dir, tg_files_dir, "institution_codes.json")
